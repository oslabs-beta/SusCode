...
async function analyzeFilesForNetworkRequests(
  filePaths: string[],
  ...
  if (results.length === 0) {
    ...
    for (let result of results) {
      ...
      if (result.originalFile) {
        ...
        const originalLocation = new vscode.Position(
          result.originalLine! - 1,
          result.originalColumn!
        );
        const originalLink = createClickableLink(
          originalUri,
          originalLocation,
          'Original Source'
        );
      ...
      const minifiedLocation = new vscode.Position(
        result.line - 1,
        result.column
      );
      const minifiedLink = createClickableLink(
        minifiedUri,
        minifiedLocation,
        'Minified Source'
      );
...
function createClickableLink(
  ...
  return `[${text}](${uri.with({
    fragment: `${position.line + 1},${position.character + 1}`,
  })})`;
}
...
async function detailedAnalysis(
  filePath: string,
  potentialRequests: PotentialRequest[]
  ...
  try {
    ...
    console.warn(
      `Failed to parse ${filePath} with determined options. Falling back to default options.`
    );
